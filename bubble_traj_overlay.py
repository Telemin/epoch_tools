...
def calculate_inj_crossing(traj_data, bubble_data):
    ...
    bubble_data = bubble_data[bubble_data[:,0].argsort()]
    bubble_times = bubble_data[:,1]
    bubble_xs = bubble_data[:,0]

    #which starts first?
    print(bubble_times.min())
    print(traj_data['time'].min())
...
def plot_all_the_things(sdf_file, window_trajs, lab_trajs, cross_points):
    print("Processing {}".format(sdf_file))
    rows,cols = (1,1)
    gs = mplgs.GridSpec(rows,cols) 
    fig = mplf.Figure(figsize=(11.7,8.3),dpi=300)
    ...
    for i,pid in enumerate(window_trajs):
        ...
        ax[0].plot(traj_x, traj_y, color=colorcycle(i), lw=1)
        ax[0].scatter(traj_x[cross_arg],traj_y[cross_arg],color=colorcycle(i))
    ...
    for i,plot in enumerate(conf['PLOTS'].values()):
        ...
        if 'xlabel' in plot:
            ax[i].set_xlabel(plot['xlabel'],size=20)

        if 'ylabel' in plot:
            ax[i].set_ylabel(plot['ylabel'],size=20)
...
def main():
    ...
    bubble_zeros = eb.find_zero_points([os.path.join(conf['sdfdir'],f) for f in
        sdf_list], 0.0)
    bubble_vel = NuCent(bubble_zeros[:,0],bubble_zeros[:,1]) 
    ...
    for sdf_file in sdf_list:
        plot_all_the_things(sdf_file, window_trajs, lab_trajs, inj_crossings)
